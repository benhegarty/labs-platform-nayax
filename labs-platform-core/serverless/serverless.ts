...
export function slsHandler(api: APIDefinition) {
  ...
  if (api.authType == AuthType.COGNITO) {
    middlewares.push(cognitoPermission({ allowedRoles: api._meta.backend.roles }));
  } else if (api.authType == AuthType.API_KEY) {
    middlewares.push(apiKey(api._meta.backend.apiKey));
  }

  // middlewares.push(
  //   validation({
  //     params: api.paramsSchema ? api.paramsSchema as ZodTypeAny : undefined,
  //     query: api.querySchema ? api.querySchema as ZodTypeAny : undefined,
  //     body: api.bodySchema ? api.bodySchema as ZodTypeAny : undefined,
  //     response: api.responseSchema ? api.responseSchema as ZodTypeAny : undefined,
  //   })
  // );
    ...
    if (api._meta.backend.tables && api._meta.backend.tables.length > 0) {
      ...
      if (api._meta.backend.tables && api._meta.backend.tables.length > 0) {
        for (const table of api._meta.backend.tables) {
          const SUFFIX = table._meta.noSuffix ? "" : environment.DYNAMODB_TABLE_SUFFIX;
          ret[api.key].iamRoleStatements!.push(
            {
              Effect: "Allow",
              Action: [
        ...
        if (api._meta.backend.s3Buckets && api._meta.backend.s3Buckets.length > 0) {
          for (const bucket of api._meta.backend.s3Buckets) {
            ret[api.key].iamRoleStatements!.push(
              {
                Effect: "Allow",
                Action: [
