...
function tableName(table: Table) {
  if (table._meta.noSuffix) return table._meta.table;
...
export async function query<T = any>(
  ...
  const params = constructQueryParams(index, query, projectedFields, filters, limit);
  ...
  do {
    ...
    const newItems = data.Items?.map(item => unmarshall(item)) as T[] || [];
...
export async function scan<T = any>(
  ...
  let items: T[] = [];
  let lastEvaluatedKey: Record<string, any> | undefined = undefined;
  const params = constructQueryParams(index, undefined, projectedFields, filters);
  // console.log(params);
  do {
    if (lastEvaluatedKey) {
      params.ExclusiveStartKey = lastEvaluatedKey; 
